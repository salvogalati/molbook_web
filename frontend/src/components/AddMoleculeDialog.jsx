...
export default function AddMoleculeDialog({ showDialog, setShowDialog, columns }) {
  ...
  return (
    ...
    >
        ...
        <div style={{display: "flex", width:"80%", justifyContent: "center", gap:"1rem"}}>
          <FloatLabel key={id === "Image" ? "SMILES": id}>
            ...
            />
            <label htmlFor={id}>{label === "Image" ? "SMILES": label}</label>
          </FloatLabel>
          { label === "Image" ? (
            <div>
            <Button icon="pi pi-pencil" onClick={() => {setShowSketcher(true)}}/>
            </div>
...
<Fieldset legend="Add new column">
        <div style={{width: "100%",display: "flex", flexDirection: "columns", gap: "2rem"}}>
            <InputText value={newColumnName} onChange={e => setNewColumnName(e.target.value)}></InputText>
            <Button icon="pi pi-plus" onClick={handleAddField}/>
        </div>
        </Fieldset>
