...
export default forwardRef(function MoleculeTable(
    ...
    if (typeof newValue === "string" && newValue.trim().length > 0) {
      try {
        ...
        if (staticFields.includes(field)) {
          ...
          body.extra_data = { ...rowData.extra_data, [field]: newValue };
        }
  ...
  prev.map((p) =>
    p.id === moleculeId
      ? {
          ...p,
          [field]: staticFields.includes(field) ? data[field] : p[field],
          extra_data: staticFields.includes(field)
            ? p.extra_data
            : { ...p.extra_data, [field]: data.extra_data?.[field] },
        }
      : p
  )
);
...
const textEditor = (options) => {
  ...
  //console.log("Editing cell:", { rowData, field, value });
  // fallback solo se value è undefined al primo render
  const initialValue =
    value !== undefined
      ? value
  ...
  const columns = [...visibleColumns, ...dynamicColumns];
        ...
        {columns.map(
          ({ field, header }) =>
            field !== "Image" && (
              <Column
                key={field}
                field={field} // rimane utile per le colonne statiche
                header={header}
                sortable
                filter
                ...
                // Solo per le colonne dinamiche leggi da extra_data
                body={
                  staticFields.includes(field)
                    ? undefined
                    : (rowData) => rowData.extra_data?.[field] ?? ""
                }
              />
            ),
        )}
